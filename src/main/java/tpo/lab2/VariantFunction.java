...
public class VariantFunction extends AbstractFunction{
    ...
    public double calculate(double x) {
        if(x<=0){
            return Math.pow(((((((csc.calculate(x)+cos.calculate(x))*tan.calculate(x)*tan.calculate(x))+sin.calculate(x)-csc.calculate(x))/(cos.calculate(x)*sec.calculate(x)))*csc.calculate(x))/csc.calculate(x)), 3);
        }
